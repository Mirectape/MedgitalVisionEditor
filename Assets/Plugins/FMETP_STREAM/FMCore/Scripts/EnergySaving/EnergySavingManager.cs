...
namespace FMSolution.FMETP
{
    public class EnergySavingManager : MonoBehaviour
    {
        ...
        private bool getAnyKeyChanged()
        {
            return Input.anyKey ? true : false;
        }
        private bool getTouchChanged()
        {
        ...
        private Vector3[] attitudeCollection;
        private Vector3 sumAttitude;
        private Vector3 tmp_averageAttitude;
        private Vector3 averageAttitude;

        private float magnitude;
        private float shakeThreshold = 0.01f;//0.02f;
        ...
        public void Action_ReloadScene(int _hr, int _min)
        {
            if (System.DateTime.Now.Hour == _hr && System.DateTime.Now.Minute == _min)
            {
                if (needReloadScene == false)
                {
                    ...
                    DelayReloadSceneAsync(_hr, _min);
                }
            }
        }
        private async void DelayReloadSceneAsync(int _hr, int _min)
        {
        ...
        public void Action_QuitApp(int _hr, int _min)
        {
            if (System.DateTime.Now.Hour == _hr && System.DateTime.Now.Minute == _min)
            {
                if (!needQuitApp)
                {
                    ...
                    needQuitApp = true;
                    DelayQuitAppAsync(_hr, _min);
                }
            }
        }
        private async void DelayQuitAppAsync(int _hr, int _min)
        {
            while (needQuitApp)
            {
                //if (System.DateTime.Now.Hour > _hr || System.DateTime.Now.Minute > _min)
